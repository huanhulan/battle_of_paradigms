import {CellLoop, Stream, StreamSink, Operational} from 'sodiumjs';
...
export default (sMouseDown: StreamSink<IPos>,
    ...
    const sSelected: Stream<Optional<IDragging>> = sMouseDown.snapshot(cDocument, (pos, doc) => {
    ...
    const sDragPending = sSelected.filterNotNull();
    ...
    const sStartDrag: Stream<Optional<IDragging>> = (sDrag.snapshot3(cPending, cDragging, ({shift, pos}, pending, dragging) => {
        ...
        const dist = getDist(pos.x, pos.y, current.startPos.x, current.startPos.y);
